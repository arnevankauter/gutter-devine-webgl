...
Warp.prototype = {
    ...
    _init: function () {
        ...
        this.input_canvas.style.display = "none";
    ...
    deform: function (settings) {
        ...
        if (settings.copy_input) {
            output_ctx.drawImage(this.input_canvas, -this.options.left, -this.options.top);
        }

        var output_img = output_ctx.getImageData(0, 0, this.output_w, this.output_h);
        ...
        for (y = 0; y < this.output_h; y++) {
            for (x = 0; x < this.output_w; x++) {
                ...
                if (dist < settings.radius) {
                    ...
                    this._warp_pixel(input_img, output_img, inp_x, inp_y, x, y, angle, dist);
                }
            }
        }
     ...
     *
     */
    _warp_pixel: function (input_img, output_img, inp_x, inp_y, x, y, angle, dist) {
        ...
        for (var i = 0; i < 4; i++) output_img.data[dest_pos + i] = input_img.data[src_pos + i];
    },
};
